...
const reviews=['service-review.json','service-review-districts.json'].map(name=>JSON.parse(fs.readFileSync(`tests/fixtures/${name}`,'utf8')) as {fixtures:{service:string;directorate:string;category:string;expected:string}[]});
test('real source-label diagnostic set: emitted labels agree with reviewed service meaning',()=>{
 let classified=0;
 for(const row of reviews.flatMap(review=>review.fixtures)){const result=classifyService(row);if(result.method==='rule'){classified++;assert.equal(result.label,row.expected,JSON.stringify(row));}}
