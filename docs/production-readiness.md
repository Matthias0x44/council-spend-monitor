...
Production dependency audit: zero known vulnerabilities at the recorded audit time. Next.js and SheetJS were updated. The Next.js/OpenNext production Worker build passed. All regression tests and both application/importer type checks pass. Lint has no errors and one pre-existing unused-variable warning in the legacy Turso helper. Automated regression coverage includes fiscal boundaries, invalid dates/amounts, refunds, repeated payment lines, idempotent imports, late headers, multiple sheets, equivalent formats, Coventry text/code selection, API validation, council budget isolation, supplier search, full CSV export, rate-limit delays and parser termination.
...
3. Check the largest councils through the built Worker and measure end-to-end D1 latency and query cost. A slow financial-year lookup was repaired; local aggregate timings are affected by the large development database and concurrent imports. A representative production D1 overview over 28,902 Coventry rows completed in 207 ms of database time; this is not a full-page latency measurement.
